...
def main(dataset,class_loss_weight,model,mkt_state,seed,gpu):
    ...
    for itr in tqdm(range(1, data_obj['n_train_batches'] * (niter + 1))):
        ...
        loss.backward()
        optimizer.step()
        average_loss.append(hawkes_loss*len(batch_dict['time_step']))
        accuracy_list.append(pred_accuracy*len(batch_dict['time_step']))
        if itr % data_obj['n_train_batches'] == 0:
            torch.cuda.empty_cache()
            logger.write('Epoch {:04d} Hawkes_loss: {:.4f} Accuracy_score: {:.4f}'.format(itr // data_obj['n_train_batches'],sum(average_loss) / len(train_record_list),sum(accuracy_list) / len(train_record_list)))
            if itr % (10 * data_obj['n_train_batches']) == 0:
                ...
                message = 'Validation Hawkes_loss {:.4f} Accuracy_score_known_t {:.4f}'.format((sum(hawkes_loss_val_list) / len(val_record_list)), accuracy_score(target_array,pred_array_known_time))
    ...
    for i in tqdm(range(data_obj['n_test_batches'])):
        ...
        hawkes_loss_test_list.append(hawkes_loss_test*len(batch_dict['time_step']))
        error_dt_test_list.append(error_dt_test*len(batch_dict['time_step']))
    target_array = np.hstack(target_array).reshape(-1)
    pred_array_known_time = np.hstack(pred_array_known_time).reshape(-1)
    pred_array_unknown_time = np.hstack(pred_array_unknown_time).reshape(-1)
    message = 'Test Hawkes_loss {:.4f} Accuracy_score_known_t {:.4f} Accuracy_score_unknown_t {:.4f} Time error {:.4f}'.format((sum(hawkes_loss_test_list) / len(test_record_list)), accuracy_score(target_array,pred_array_known_time), accuracy_score(target_array,pred_array_unknown_time), (sum(error_dt_test_list) / len(test_record_list)))
    logger.write(message)
    print(message)
    logger.close()
